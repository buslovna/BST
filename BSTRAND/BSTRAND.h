...
namespace cop3530 {
	...
	template <typename K, typename V, bool(*comp)(K, K), bool(*eq)(K, K)>
	void
		BSTRAND<K, V, comp, eq>::clear(Node * node){
		if (node == NULL)
			return;
		clear(node->left);
		clear(node->right);
  ...
  template <typename K, typename V, bool(*comp)(K, K), bool(*eq)(K, K)>
  void
  BSTRAND<K, V, comp, eq>::print_inorder(Node * node)
  {
  	if (node->left)
	...
	template <typename K, typename V, bool(*comp)(K, K), bool(*eq)(K, K)>
	void
		BSTRAND<K, V, comp, eq>::insert(K key, V value) {
		int num = rand() % 5;
		Node * temp;
	...
	template <typename K, typename V, bool(*comp)(K, K), bool(*eq)(K, K)>
	void
		BSTRAND<K, V, comp, eq>::remove(K key) {
		if (is_empty())
	...
	template <typename K, typename V, bool(*comp)(K, K), bool(*eq)(K, K)>
	unsigned int
		BSTRAND<K, V, comp, eq>::size_aux(Node * node)
	{
		if (!node)
			return 0;
		else
